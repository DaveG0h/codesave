//doubly linked list for closed addressing hash table
#include <stdio.h>
#include <stdlib.h>
...
int HashInsert(HashTable* Q1Ptr, int key)
{
    ...
    //hash
    indexed = Hash(key, Q1Ptr->hSize);
    temp->key = key;
    temp->next = NULL;
    temp->pre = NULL;
    //if .head = null, add
    if(Q1Ptr->Table[indexed].head == NULL)
    {
        Q1Ptr->Table[indexed].head = temp;
        Q1Ptr->Table[indexed].size++;
        return 1;
    }
    //if collision, add infront of the doubly link list
    else
    {
        temp->next = Q1Ptr->Table[indexed].head;
        Q1Ptr->Table[indexed].head->pre = temp;
        Q1Ptr->Table[indexed].head = temp;
        Q1Ptr->Table[indexed].size++;
        return 1;
...
int HashDelete(HashTable* Q1Ptr, int key)
{
    ...
    if(temp->key == key)
    {
        ...
        Q1Ptr->Table[indexed].head = temp->next;
        free(temp);
        Q1Ptr->Table[indexed].size--;
        return 1;
    }
