...
{
	// add your code here
	int frontsize, backsize, indexed=0, tempitem;
	ListNode *tempcur;

	if ((ll->size%2) == 1) //if odd number of ll nodes
	{
		backsize = (ll->size)/2;
		frontsize = backsize+1;
	}
	else //even
	{
		backsize = (ll->size)/2;
	...
	tempcur = ll->head;
	for(frontsize; frontsize>0 && tempcur!=NULL ; frontsize--, indexed++)
	{
		tempitem = tempcur->item;
		tempcur = tempcur->next;
		insertNode(resultFrontList, resultFrontList->size, tempitem);
		if(removeNode(ll, indexed) == 0)
		{
			indexed--;
		}
	...
	for(backsize; backsize>0 && tempcur!=NULL ; backsize--, indexed++)
	{
		tempitem = tempcur->item;
		tempcur = tempcur->next;
		insertNode(resultBackList, resultBackList->size, tempitem);
		if(removeNode(ll, indexed) == 0)
		{
			indexed--;
		}
